...
public class JedisAdaptor implements InitializingBean {
    ...
    public void followTransaction(int userID, int entityID) {
    ...
    public void unfollowTransaction(int userID, int entityID) {

        try (Jedis jedis = pool.getResource()) {
            Transaction transaction = new Jedis().multi();
            String key = RedisKeyUtil.getBizFollowlistKey(userID);
            jedis.zrem(key, String.valueOf(entityID));
            jedis.zrem(RedisKeyUtil.getBizFanslistKey(entityID), String.valueOf(userID));
            transaction.exec();
        } catch (Exception e) {
            logger.info("发生异常: " + e.getMessage());
        }
